...
export default function CodeGrid() {
    ...
    return (
        <>
            <div className={`w-screen h-screen fixed bg-black/90 top-0 left-0 flex ${contextInformation.isShown ? 'scale-100' : 'scale-0'} transition-all`}>
            ...
            {isMobile && <p className="text-xs italic">Hold your finger or right click anything to know what it is!</p>}
            <div className="grid grid-cols-3 select-none [-webkit-touch-callout: none] [-webkit-user-select: none] gap-2 xl:max-h-120 max-h-80 p-2 scroll-smooth scrollbar-thumb-(--border-color) overflow-x-hidden overflow-scroll ring-2 ring-(--border-color)">

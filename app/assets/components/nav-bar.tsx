...
function Logo() {
    ...
    return (
        <Link href="/" className={`text-2xl w-fit select-none flex gap-4 ${isRootPath ? "bg-white text-black" : "hover:bg-white hover:text-black text-white active:bg-white"} transition items-center p-2`} transitionTypes={['nav-forward']}>
...
function PanelButton({ isMobileMode, setIsOpen, isOpen }: { isMobileMode: boolean, setIsOpen: Dispatch<SetStateAction<boolean>>, isOpen: boolean }) {
    return <>
        {
            isMobileMode &&
...
function TopBarLinks({ isMobileMode, setIsOpen, isOpen }: { isMobileMode: boolean, setIsOpen: Dispatch<SetStateAction<boolean>>, isOpen: boolean }) {
    return <>
...
export const NavBar = () => {
    ...
    return <div className="bg-black w-full min-h-20 border-(--border-color) border-b-5 p-3 flex flex-row items-center gap-4">
        {isLoading ?
            ...
            <>
                {
                    isMobile
                        ...
                        <>
                            ...
                            <div className={`fixed ${isOpen ? "translate-x-0" : "-translate-x-full"} left-0 transition-all bg-black min-w-50 flex flex-col gap-2 p-5 h-full top-0 border-r-3 border-(--border-color)`}>
